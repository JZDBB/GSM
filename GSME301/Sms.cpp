...
BOOL testSignal(int &rssi, int &ber) {
	char ans[128];		// 应答串
	char testat[8];
	testat[0] = 'A';
	testat[1] = 'T';
	...
	try {
		rssi = atoi(strtok(ans + 8, ","));
		ber = atoi(strtok(NULL, "\0"));
	}
	catch(int i){
		return false;
	}
...
BOOL testSIMCARD()
{
	...
	for (int i = 0; i<1000; i++);
	ReadComm(ans, 128);
	if (strstr(ans, "+CSMINS: 0,1") == NULL)  return FALSE;
	return true;
}
...
BOOL gsmTMENU(int &now, int &total)
{
	char ans[128];		// 应答串
	char testat[14];
	testat[0] = 'A';
...
BOOL gsmTMENU2(int &now, int &total)
{
	char ans[128];		// 应答串
	char testat[10];
	testat[0] = 'A';
	...
	ReadComm(ans, 128);
	try {
		now = atoi(strtok(ans + 14, ","));
		total = atoi(strtok(NULL, "\r"));
		//throw 1;
	}
	catch (int i) {
		return false;
